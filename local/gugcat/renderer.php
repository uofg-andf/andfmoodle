...
class local_gugcat_renderer extends plugin_renderer_base {
    ...
    public function display_aggregation_tool($rows, $activities) {
        ...
        foreach ($rows as $row) {
            ...
            ? html_writer::start_tag('td').'('.number_format($row->aggregatedgrade->rawgrade, 3, '.', '').')'.$this->context_actions($row->studentno, null, true, $courseformhistoryparams, true).html_writer::end_tag('td')
            : html_writer::tag('td', '('.number_format($row->aggregatedgrade->rawgrade, 3, '.', '').')');
            $htmlrows .= html_writer::end_tag('tr');
        }
