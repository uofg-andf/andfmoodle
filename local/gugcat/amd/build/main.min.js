        ...
        init: function() {
            ...
            if(GCAT){
                ...
                if(input_percentarr.length > 0){
                    ...
                    // Insert checkbox
                    var cb_fields = document.querySelectorAll('.input-cb');
                    if(cb_fields.length > 0){
                        var labels = document.querySelectorAll('.input-cb div > label');
                        for (const [id, lbl] of labels.entries()) {
                            var checkbox = document.createElement('input');
                            checkbox.type = 'checkbox';
                            checkbox.name = `assessment[${id}]`;
                            checkbox.className  = 'cb-assessment';
                            lbl.prepend(checkbox);
                        }
                    }
                    input_percentarr.forEach(div => {
                        ...
                        input.addEventListener('input', (e) => {
                            let total = 0;
                            input_percentarr.forEach(div => total += parseInt(div.querySelector('input[type="text"]').value));
